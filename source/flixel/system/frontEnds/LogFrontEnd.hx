	...
	public function advanced(Data:Dynamic, ?Style:LogStyle, FireOnce:Bool = false):Void
	{
        ...
        */
        if (CoolUtil.isDevMode()) {
            ...
            var prefix = "[FLIXEL]";
            if (Style == LogStyle.CONSOLE)  prefix = "> ";
            if (Style == LogStyle.ERROR)    prefix = "[FLIXEL ERROR]";
            if (Style == LogStyle.NORMAL)   prefix = "[FLIXEL]";
            if (Style == LogStyle.NOTICE)   prefix = "[FLIXEL NOTICE]";
            if (Style == LogStyle.WARNING)  prefix = "[FLIXEL WARNING]";
