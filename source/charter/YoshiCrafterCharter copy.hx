...
class YoshiCrafterCharter extends MusicBeatState {
    ...
    function getSectionIndexFor(t:Float) {
        var beat = Std.int(getStepsFor(t) / 4);

        return Math.floor(t / 4);
    }

	function getSectionFor(t:Float) {
        var sec = _song.notes[getSectionIndexFor(t)];
        if (sec == null) sec = _song.notes[Math.floor(t / 4)] = {
            altAnim: false,
            mustHitSection: true,
            sectionNotes: [],
            lengthInSteps: 16,
    ...
    function getStepsFor(t:Float):Float {
        var steps:Float = 0;

        var oldSongTime:Float = 0;
        var stepCrochet:Float = (60 / _song.bpm) * 250;
        
        for(e in Conductor.bpmChangeMap) {
            if (e.songTime > t) {
                break;
            } else {
                steps += Std.int((e.songTime - oldSongTime) / stepCrochet);
    
                stepCrochet = (60 / e.bpm) * 250;
                oldSongTime = e.songTime;
            }
        }
        var nT = t - oldSongTime;
        steps += nT / stepCrochet;
    ...
    public function compile() { // out of ideas for a func name
        ...
        for(s in notes) {
            if (s.noteData >= 0) {
                ...
                var section = getSectionFor(s.strumTime);
                // if (section == null) {
    ...
    public function getSectionStartFromTime(t:Float) {
        
    }

    public function getSectionStart(sec:Int) {
        var steps = sec * 16;
        var addedSteps:Float = 0;
        var lastStepTime:Float = 0;
        for(e in Conductor.bpmChangeMap) {
            if (addedSteps + (e.stepTime - lastStepTime) > steps) {
                break;
            }
            lastStepTime = e.stepTime;
        }
    }
    ...
    public function create_ui() {
        ...
        var deleteSectionButton = new FlxUIButton(-GRID_SIZE - 5, 0, "", function() {
            var min = getSectionStartFromTime(Conductor.songPosition);
            var max = getSectionEndFromTime(Conductor.songPosition) + 1;
        ...
        var copySectionButton = new FlxUIButton(deleteSectionButton.x, deleteSectionButton.y + deleteSectionButton.height + 5, "", function() {
            var sec = getSectionIndexFor(Conductor.songPosition);
            copiedSection = sec;
        });
        ...
        var pasteSectionButton = new FlxUIButton(copySectionButton.x, copySectionButton.y + copySectionButton.height + 5, "", function() {
            ...
            if (copiedSection != sec && copiedSection >= 0) {
                var newMin = getSectionStartFromTime(Conductor.songPosition);
                var newMax = getSectionEndFromTime(Conductor.songPosition);
                var min = getSectionStart(copiedSection * Conductor.crochet * 4);
                var max = getSectionEnd((copiedSection + 1) * Conductor.crochet * 4);
        ...
        var swapSectionButton = new FlxUIButton(pasteSectionButton.x, pasteSectionButton.y + pasteSectionButton.height + 5, "", function() {
            var min = getSectionStartFromTime(Conductor.songPosition);
            var max = getSectionEndFromTime(Conductor.songPosition);
            for(n in notes) {
                if (n.strumTime >= min && n.strumTime < max) {
    ...
    public override function update(elapsed:Float) {
        ...
        copyPasteButtonsContainer.y = (GRID_SIZE * 16) * getSectionIndexFor(Conductor.songPosition) * zoom;
        if (playing) {
        ...
        voicesWaveform.y = instWaveform.y = ((getSectionIndexFor(Conductor.songPosition) * GRID_SIZE * 16) + (-GRID_SIZE * 16)) * zoom;
        for(waveform in [instWaveform, voicesWaveform]) {
            waveform.scale.set(1, zoom);
            waveform.updateHitbox();
            waveform.antialiasing = zoom < 1;
            if (waveform.visible) {
                var curSection = getSectionIndexFor(Conductor.songPosition);
                if (curSection != (waveform == instWaveform ? instWaveformSection : voicesWaveformSection)) {
                    ...
                    waveform.generateFlixel(Std.int(getSectionStartForTime(Conductor.songPosition)), Std.int(getSectionEndForTime(Conductor.songPosition)));
                }
            }
        }
        ...
        if (sec != sectionTabSection) {
            ...
            if (changeBPMSection_bpm.value != Conductor.bpm) Conductor.changeBPM(Std.int(changeBPMSection_bpm.value));
        }
        ...
        if (section != null) {
            // the most confusing line of code i've ever wrote
            if (section.bpm != Std.int(changeBPMSection_bpm.value)) {
                // bpm changes
                Conductor.mapBPMChanges(_song);
            }
            section.bpm = Std.int(changeBPMSection_bpm.value);
        ...
        followThing.y = getPosForTime(Conductor.songPosition);
