...
class CreditsState extends MusicBeatState {
    ...
    public override function create() {
        var bg:FlxSprite = new FlxSprite(-80).loadGraphic(Paths.image('menuBGCrafter'));
		bg.scrollFactor.x = 0;
		bg.scrollFactor.y = 0;
        ...
        for (mod in mods) {
            ...
            if (Assets.exists(path)) {
                ...
                if (json != null) {
                    y++;
                    var modTitle:Alphabet = new Alphabet(0, 125 * y, (mod == "\\") ? "Crafter Engine" : ModSupport.getModName(mod), true, false);
