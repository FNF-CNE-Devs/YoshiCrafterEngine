	...
	private function keyShit():Void
	{
		...
		if ((upP || rightP || downP || leftP) && generatedMusic)
		{
			...
			notes.forEachAlive(function(daNote:Note)
			{
				...
				{
					if (controlArray[daNote.noteData % SONG.keyNumber]) {
						var can = true;
						if (notesToHit[daNote.noteData % SONG.keyNumber] != null) if (notesToHit[daNote.noteData % SONG.keyNumber].strumTime < daNote.strumTime) can = false;
