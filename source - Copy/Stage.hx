...
class Limo extends FNFStage
{
	...
	override function createAfterGf()
	{
		PlayState.current.add(limo);
	}

	override function createAfterChars()
	{
		PlayState.current.boyfriend.y -= 220;
		PlayState.current.boyfriend.x += 260;

		resetFastCar();
		PlayState.current.add(fastCar);
	}
	...
	override function create()
	{
		...
		PlayState.current.add(bgLimo);
		BackgroundDancer.shrek = PlayState.SONG.song.toLowerCase() == "smash-mouth-milf";
		grpLimoDancers = new FlxTypedGroup<BackgroundDancer>();
		PlayState.current.add(grpLimoDancers);
...
class Spooky extends FNFStage
{
	...
	public override function create()
	{
		...
		PlayState.current.add(halloweenBG);
	}
	...
	function lightningStrikeShit():Void
	{
		...
		PlayState.current.boyfriend.playAnim('scared', true);
		PlayState.current.gf.playAnim('scared', true);
	}
}
class Mall extends FNFStage
{
	...
	public override function create() {
		bfOffset = new FlxPoint(200, 0);
		PlayState.current.defaultCamZoom = 0.80;
		...
		PlayState.current.add(upperBoppers);
		...
		PlayState.current.add(bgEscalator);

		var tree:FlxSprite = new FlxSprite(370, -250).loadGraphic(Paths.image('christmas/christmasTree'));
		tree.antialiasing = true;
		tree.scrollFactor.set(0.40, 0.40);
		PlayState.current.add(tree);
		...
		PlayState.current.add(bottomBoppers);

		var fgSnow:FlxSprite = new FlxSprite(-600, 700).loadGraphic(Paths.image('christmas/fgSnow'));
		fgSnow.active = false;
		fgSnow.antialiasing = true;
		PlayState.current.add(fgSnow);

		santa = new FlxSprite(-840, 150);
		santa.frames = Paths.getSparrowAtlas('christmas/santa');
		santa.animation.addByPrefix('idle', 'santa idle in fear', 24, false);
		santa.antialiasing = true;
		PlayState.current.add(santa);
	}
...
class MallEvil extends FNFStage
{
	public override function create()
	{
		bfOffset = new FlxPoint(320, 0);
		dadOffset = new FlxPoint(0, -80);
		super.create();
		...
		PlayState.current.add(evilTree);

		var evilSnow:FlxSprite = new FlxSprite(-200, 700).loadGraphic(Paths.image("christmas/evilSnow"));
		evilSnow.antialiasing = true;
		PlayState.current.add(evilSnow);
	}
}
...
{
	public override function create() {
		// Kade forgot to disable this lol
