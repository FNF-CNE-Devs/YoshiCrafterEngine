...
class Song
{
	...
	public static function loadModFromJson(jsonInput:String, mod:String, ?folder:String):SwagSong
	{
		var rawJson = "";
		if (folder == null)
			rawJson = File.getContent(Paths.getModsFolder() + '/$mod/assets/data/$jsonInput/$jsonInput.json');
		else
			rawJson = File.getContent(Paths.getModsFolder() + '/$mod/assets/data/$folder/$jsonInput.json');
