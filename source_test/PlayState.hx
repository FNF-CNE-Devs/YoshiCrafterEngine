...
#if note_opti
	import Note as Note_;
	import Notes.OptiNote as Note;
#else
	import Note as Note_;
#end
...
class PlayState extends MusicBeatState
{
	...
	public static var instance(get, set):PlayState;
	public static function get_instance() {return current;};
	public static function set_instance(c:PlayState) {return current = c;};
	...
	private function generateSong(dataPath:String):Void
	{
		...
		for(t in PlayState.SONG.noteTypes) {
			...
			var calculateNote = function() {
				var note:Note_ = script.getVariable("note");
		...
		#if note_opti
			for(i in 0...(SONG.keyNumber * 2 * SONG.noteTypes.length)) {
				notes.add(new Note_(0, i));
			}
			notes.settingUp = false;
		#end
		for (section in noteData)
		{
			...
			for (songNotes in section.sectionNotes)
			{
				...
				if (notes.currentNotes.length > 0)
					oldNote = notes.currentNotes[notes.currentNotes.length - 1];
				else
					oldNote = null;
				...
				// if (engineSettings.downscroll) ;
				#if opti_note
					notes.currentNotes.push(swagNote);
				#else
					notes.add(swagNote);
				#end
		...
		if (inCutscene) {
			...
			if (generatedMusic)
			{
				notes.forEachAlive(function(daNote)
				{
					...
					if (strum == null) strum = (daNote.mustPress ? playerStrums.members : cpuStrums.members)[(daNote.noteData % _SONG.keyNumber) % SONG.keyNumber];
	...
	private function keyShit(elapsed:Float):Void
	{
		...
		if (!engineSettings.botplay) {
			...
			// BOTPLAY CODE
			justPressedArray = [for (i in 0...SONG.keyNumber) false];
			notes.forEachAlive(function(daNote:Note_)
				{
		...
		if ((justPressedArray.indexOf(true) != -1) && generatedMusic) // smart ass code lmao
		{
			...
			var notesToHit:Array<Note_> = [for (i in 0...SONG.keyNumber) null];

			var additionalNotesToHit:Array<Note_> = [];
	...
	function goodNoteHit(note:Note_):Void
	{
		if (!note.wasGoodHit)
		{
